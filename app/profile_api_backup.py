...
async def get_vessel_profile(request: Request, imo_number: str, base_date: Optional[str] = None):
    ...
    try:
        ...
        # 获取船舶基本信息（从第一条轨迹记录）
        vessel_info = tracks_collection.find_one({"imo_number": imo_number})
        if not vessel_info:
            return JSONResponse({
                "success": False,
                "msg": "未找到该船舶"
        ...
        profile = {
            ...
            "ship_type": vessel_info.get("ship_type", "Unknown"),
...
if __name__ == "__main__":
    print("船舶画像API已加载")
    print("可用端点：")
    print("  GET /api/profile/overview - 获取船舶画像总览")
    print("  GET /api/profile/vessel/{imo_number} - 获取指定船舶画像")
    print("  GET /api/profile/statistics - 获取统计数据")
