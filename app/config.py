...
# 替换成你自己的默认值，或者通过环境变量覆盖
class Settings:
    """应用配置"""
    # PostgreSQL配置
    db_host = os.getenv("SILVERNAV_DB_HOST", "")
    ...
    admin_password = os.getenv("SILVERNAV_DB_ADMIN_PASSWORD", "")

    read_user = os.getenv("SILVERNAV_DB_READ_USER", "")
    read_password = os.getenv("SILVERNAV_DB_READ_PASSWORD", "")

    # MongoDB配置
    mongo_host = os.getenv("SILVERNAV_MONGO_HOST", "")
    ...
    # Redis配置
    redis_host = os.getenv("SILVERNAV_REDIS_HOST", "")
    redis_port = int(os.getenv("SILVERNAV_REDIS_PORT", "6379"))
    redis_db = int(os.getenv("SILVERNAV_REDIS_DB", "0"))
    redis_password = os.getenv("SILVERNAV_REDIS_PASSWORD", "")

    # Kafka配置
    kafka_bootstrap_servers = os.getenv("SILVERNAV_KAFKA_SERVERS", ":9092")
